// C++는 복사 생성자(Copy Constructor)는 다른 인스턴스의 참조(Reference)를 인수로 받아서, 자신의 인스턴스를 초기화할 수 있도록 한다.
// 깊은 복사(Deep Copy)를 이용해 만들어진 인스턴스는 기존의 인스턴스와 다른 메모리 공간에 할당되어 독립적이다.

// 얕은 복사는 원래 객체와의 메모리 주소를 공유하는 방식의 복사이다.
...
// 참조형은 접근할 때 암시적으로 역참조되는 포인터와 같은 역할을 한다. (참조형은 컴파일러 내부적으로 포인터를 사용하는 방식으로 구현된다.)
...
int main(void){
	Character character1 = Character("Monster1", 10, 20);
	character1.pointUp();
