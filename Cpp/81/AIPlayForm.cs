...
namespace Client
{
    public partial class AIPlayForm : Form
    {
		...
		/*
		추가로 만들어진 변수
		limit은 내다보고자 하는 수의 개수 - DFS를 종료하고자하는 기점
		targetX, targetY는 인공지능이 찾은 최적의 수
		*/
		...
		*/
        private void refresh()
        {
            ...
            int rand = new Random().Next(1, 3); //1 혹은 2를 내보내는 random 수
		...
		/*
		AlphaBetaPruning : AI가 발견한 최적의 수
		*/
		...
		// minimax에서 max의 역할이 alpha이다. 그래서 alpha에는 절대값이 큰 수를 넣어줘야한다.
        int AlphaBetaPruning(int level, int alpha, int beta)
        {
            ...
            // MAX 부분에 해당하는 경우. level은 0부터 시작한다. - 인공지능 입장
            if (level % 2 == 0)
            {
